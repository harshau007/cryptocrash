...
import { Bet, IBet, IRound, Player, Round, Transaction } from "./database";
...
export class GameEngine {
  ...
  crashPoint: number;
  activeBets: Map<string, IBet>;
  roundInterval: NodeJS.Timeout | null;
  multiplierInterval: NodeJS.Timeout | null;
  pendingBets: Map<string, IBet>;
  roundDuration: number;
  maxMultiplier: number;
  growthRate: number;

  constructor(io: IOServer, cryptoService: CryptoService) {
    this.io = io;
  ...
  public async createRound(data: any): Promise<IRound> {
  ...
  public async processPendingBets(): Promise<void> {
  ...
  public async loadActiveBets(): Promise<void> {
  ...
  public startMultiplierUpdates(): void {
  ...
  public async crashRound(): Promise<void> {
    this.isActive = false;
  ...
  public calculateCrashPoint(hash: string): number {
  ...
  public async processLostBets(): Promise<void> {
  ...
  public async updateRoundStatus(): Promise<void> {
    if (!this.currentRound) return;
  ...
  public async cashOut(playerName: string, betId: string): Promise<any> {
    if (!this.isActive) {
