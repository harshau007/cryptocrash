...
import { Bet, Player, Round, Transaction } from "../core/database.js";
export default (gameEngine, cryptoService) => {
  const router = Router();
  ...
  router.post("/bet", validateBetInput, async (req, res) => {
    try {
      const { playerName, usdAmount, cryptocurrency } = req.body;
  ...
  router.get("/wallet/:playerName", async (req, res) => {
    try {
      ...
      const btcUsd = player.wallet.BTC * (prices.BTC || 0);
      const ethUsd = player.wallet.ETH * (prices.ETH || 0);
      const wallet = {
  ...
  router.get("/transactions/:playerName", async (req, res) => {
    try {
